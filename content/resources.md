---
title: Resources
date: 1999-01-01
modified: 1999-01-01
description: A curated collection of tools, frameworks, and regulatory guides for the Malaysian CTI practitioner.
---
# The Rectifyq Toolbox

To move from "collecting data" to "producing intelligence," you need the right tools and a structured approach. This page centralizes the resources we use at Rectifyq to track and analyze the Malaysian threat landscape.

---
## 📕Notion
🗓️[Malaysia Cybersecurity Events](https://rectifyq.notion.site/Malaysia-Cybersecurity-Events-e5b7d360750c42cd8d35de22ab0ff8e1)\
🔁[Cybersecurity Subscription Comparison](https://rectifyq.notion.site/Subscription-Comparison-2aaba5ae66c780f4902dc59212d57fae)\
💼[Certification Comparison and Action Plan (CCAP)](https://rectifyq.notion.site/Certification-Comparison-and-Action-Plan-CCAP-1ac3036cc51647679fb217433dc90f0a)

---
## 💻Github Repos

### Collections
- [List of Computer Crime Act 1997 Cases](https://github.com/rectifyq/Collections/blob/main/Computer-Crime-Act-1997-Cases/Computer-Crime-Act-1997-Cases.csv)
- [List of Phishing Targeting Malaysia](https://github.com/rectifyq/Collections/tree/main/Phishing-Targeting-Malaysia)
- [Diamond Models created](https://github.com/rectifyq/Collections/tree/main/Diamond-Models)

### CTFd to MISP Importer
- [CTFd-to-MISP](https://github.com/rectifyq/ctfd-to-misp)

---

## 💬MISP Feed (Free Integration - TLP:Clear only)
### 🌏Global MISP events
- MISP 2024 - [https://feeds.rectifyq.com/MISP2024](https://feeds.rectifyq.com/MISP2024)
- MISP 2025 - [https://feeds.rectifyq.com/MISP2025](https://feeds.rectifyq.com/MISP2025)
- MISP 2026 - [https://feeds.rectifyq.com/MISP2026](https://feeds.rectifyq.com/MISP2026)

### 🇲🇾 MISP MY
- MISP MY - soon

### 📦Miscelleanous
- MISP ICS-OT - [https://feeds.rectifyq.com/MISP-ICS-OT](https://feeds.rectifyq.com/MISP-ICS-OT)

---
# Malaysia Cybersecurity Ecosystem
```mermaid
flowchart TB
    direction RL
    subgraph CTF
        direction RL
        C1[Wargames.my]
        C2[GCTF]
        C3[Bahtera Siber]
        C4[Sherpasec CTF]
        C5[I-Hack]
        C6[ABOH]
        C7[UTPHax]
        C8[Hack@10]
        C9[SunCTF]
        C10[KPMG]
        C11[iCTFF]
        C12[LIGA CTF]
    end
    subgraph Community
        direction RL
        M1[Rawsec]
        M0[RE:HACK CSLU]
        M2[Sherpasec]
        M3[Offsec]
        M4[ISC2]
        M5[OWASP KL]
        M6[MyOPECS]
        M7[MOC]
        M8[RE:UN10N]
        M9[PINCER]
        M10[I/O–TRIX]
    end
    subgraph Conferences/Camps
        direction RL
        F0[MCC]
        F1[CyberDSA]
        F2[Cydes]
        F3[NanoSec]
        F4[5GOT]
        F5[PH Talks]
        F6[RISE]

    end


    click M1 "https://mcco.org.my/" _blank
    click M0 "https://www.rehack.xyz/c/cslu.html" _blank
    click M2 "https://sherpasec.org/" _blank
    click M3 "https://www.linkedin.com/groups/14716092/" _blank
    click M4 "https://www.isc2chapter.my/" _blank
    click M5 "https://owasp.org/www-chapter-kuala-lumpur/" _blank
    click M6 "https://t.me/+HZLe3mJs9MxlNzRl" _blank
    click M7 "https://www.linkedin.com/company/malaysiaosintcommunity/" _blank
    click M8 "https://x.com/reun10n_" _blank
    click M9 "https://www.linkedin.com/groups/14716288/" _blank
    click M10 "https://www.linkedin.com/company/i-o-trix/" _blank

    click C1 "https://wargames.my" _blank
    click C2 "https://girls-in-ctf.online/" _blank
    click C3 "https://x.com/bahterasiber" _blank
    click C4 "https://sherpasec.org/sherpactf/" _blank
    click C5 "https://news.uitm.edu.my/2024/07/ihack-2024-and-siber-siaga-2024-malaysia-takes-strides-in-cybersecurity-education-and-awareness/" _blank
    click C6 "https://www.linkedin.com/posts/apumalaysia_the-asean-battle-of-hackers-aboh-2023-activity-7147881717654036481-ViCf/" _blank
    click C7 "https://utphax.github.io/" _blank
    click C8 "https://www.instagram.com/hack10.uniten/" _blank
    click C9 "https://www.linkedin.com/company/csc-sunway/" _blank
    click C10 "https://www.youtube.com/watch?v=xSIKCjKj2gY" _blank
    click C11 "https://www.ictff.com.my/" _blank
    click C12 "https://appsecmy.com/pages/liga-ctf-2026" _blank

    click F0 "https://cybercamp.my/" _blank
    click F1 "https://www.cyberdsa.com/" _blank
    click F2 "https://cydes.my/" _blank
    click F3 "https://nanosec.asia/" _blank
    click F4 "https://5got.asia/" _blank
    click F5 "https://phtalks.ptsecurity.com/kualalumpur" _blank
    click F6 "https://www.linkedin.com/posts/team-cymru_rise-malaysia-2025-activity-7376322136908558336-gMy3" _blank
```

```mermaid
flowchart TB
    %% Layout Configuration %%
    classDef default fill:#fafafa,stroke:#333,stroke-width:1px;
    classDef group fill:#f9f9f9,stroke:#ddd,stroke-dasharray: 5 5;
    
    %% --- CONFERENCES & CAMPS --- %%
    subgraph Conferences["Conferences & Camps"]
        direction TB
        F0[MCC] --- F1[CyberDSA] --- F2[Cydes]
        F3[NanoSec] --- F4[5GOT] --- F5[PH Talks] --- F6[RISE]
        
        %% Structure Rows
        F0 ~~~ F3
    end

    %% --- COMMUNITIES --- %%
    subgraph Community["Communities"]
        direction TB
        M1[Rawsec] --- M0[RE:HACK CSLU] --- M2[Sherpasec] --- M3[Offsec]
        M4[ISC2] --- M5[OWASP KL] --- M6[MyOPECS] --- M7[MOC]
        M8[RE:UN10N] --- M9[PINCER] --- M10[I/O–TRIX]
        
        %% Structure Rows
        M1 ~~~ M4 ~~~ M8
    end

    %% --- CTF PLATFORMS --- %%
    subgraph CTF["CTF Competitions"]
        direction TB
        C1[Wargames.my] --- C2[GCTF] --- C3[Bahtera Siber] --- C4[Sherpasec CTF]
        C5[I-Hack] --- C6[ABOH] --- C7[UTPHax] --- C8[Hack@10]
        C9[SunCTF] --- C10[KPMG] --- C11[iCTFF] --- C12[LIGA CTF]
        
        %% Structure Rows
        C1 ~~~ C5 ~~~ C9
    end

    %% --- Hyperlinks --- %%
    click M1 "https://mcco.org.my/" _blank
    click M0 "https://www.rehack.xyz/c/cslu.html" _blank
    click M2 "https://sherpasec.org/" _blank
    click M3 "https://www.linkedin.com/groups/14716092/" _blank
    click M4 "https://www.isc2chapter.my/" _blank
    click M5 "https://owasp.org/www-chapter-kuala-lumpur/" _blank
    click M6 "https://t.me/+HZLe3mJs9MxlNzRl" _blank
    click M7 "https://www.linkedin.com/company/malaysiaosintcommunity/" _blank
    click M8 "https://x.com/reun10n_" _blank
    click M9 "https://www.linkedin.com/groups/14716288/" _blank
    click M10 "https://www.linkedin.com/company/i-o-trix/" _blank

    click C1 "https://wargames.my" _blank
    click C2 "https://girls-in-ctf.online/" _blank
    click C3 "https://x.com/bahterasiber" _blank
    click C4 "https://sherpasec.org/sherpactf/" _blank
    click C5 "https://news.uitm.edu.my/2024/07/ihack-2024-and-siber-siaga-2024-malaysia-takes-strides-in-cybersecurity-education-and-awareness/" _blank
    click C6 "https://www.linkedin.com/posts/apumalaysia_the-asean-battle-of-hackers-aboh-2023-activity-7147881717654036481-ViCf/" _blank
    click C7 "https://utphax.github.io/" _blank
    click C8 "https://www.instagram.com/hack10.uniten/" _blank
    click C9 "https://www.linkedin.com/company/csc-sunway/" _blank
    click C10 "https://www.youtube.com/watch?v=xSIKCjKj2gY" _blank
    click C11 "https://www.ictff.com.my/" _blank
    click C12 "https://appsecmy.com/pages/liga-ctf-2026" _blank

    click F0 "https://cybercamp.my/" _blank
    click F1 "https://www.cyberdsa.com/" _blank
    click F2 "https://cydes.my/" _blank
    click F3 "https://nanosec.asia/" _blank
    click F4 "https://5got.asia/" _blank
    click F5 "https://phtalks.ptsecurity.com/kualalumpur" _blank
    click F6 "https://www.linkedin.com/posts/team-cymru_rise-malaysia-2025-activity-7376322136908558336-gMy3" _blank
```



